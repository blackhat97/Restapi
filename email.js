...
const fromAddress = 'noreply@webcomics.pk';
...
const sendPasswordResetEmail = async (targetEmail, accountID) => {
    ...
    const url = `https://webcomics.pk/reset/${token}`;
    try {
        await sgMail.send({
            to: targetEmail,
            from: fromAddress,
...
const sendVerificationEmail = async (targetEmail, accountID) => {
    ...
    const url = `https://webcomics.pk/verify/${token}`;
    try {
        await sgMail.send({
            to: targetEmail,
            from: fromAddress,
            ...
            html: `
<p>Thank you for creating an account on Webcomics.</p>
